...
# Detect via the HTML-comment header emitted at the top of every generated
# CLAUDE.md. Anchoring the marker to `<!--` dramatically reduces false
# positives (hand-written project CLAUDE.md that mentions the generator by
# name in prose won't trigger). Skipping here prevents prp-install-all from
# overwriting the agent's identity-AGENTS.md on every run.
#
# Fail-safe design: grep exit 2 (permission denied / I/O error / stale
# filesystem) is treated identically to exit 0 (match) — we skip the
# install rather than risk silently overwriting a CLAUDE.md-marked identity
# file that we just couldn't read. An install-not-done is recoverable with
# a re-run; an overwrite is destructive without git history.
SOUL_ORCHESTRA_MARKER="<!-- Generated by soul-orchestra generate-claude-md.py"
_skip_agents_md=false
if [ -f "$PROJECT_DIR/CLAUDE.md" ]; then
    grep -qF "$SOUL_ORCHESTRA_MARKER" "$PROJECT_DIR/CLAUDE.md" && _grep_exit=0 || _grep_exit=$?
    if [ "$_grep_exit" -eq 0 ]; then
        _skip_agents_md=true
    elif [ "$_grep_exit" -eq 2 ]; then
        echo -e "${YELLOW}  ⚠️  Cannot read $PROJECT_DIR/CLAUDE.md (grep exit 2) — skipping AGENTS.md install to avoid overwriting a possible soul-orchestra agent identity file.${NC}" >&2
        _skip_agents_md=true
    fi
    # exit 1 (no match) → _skip_agents_md stays false → install proceeds
fi
if $_skip_agents_md; then
    echo "→ Generic (AGENTS.md) — skipped: soul-orchestra agent home (owns root AGENTS.md)"
else
