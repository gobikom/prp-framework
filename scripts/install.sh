...
# Detect by grepping the self-identifying header in PROJECT_DIR/CLAUDE.md.
# Skipping here prevents prp-install-all from overwriting the agent's
# identity-AGENTS.md on every run in ~/repos/agents/agent-{id}/ homes.
SOUL_ORCHESTRA_MARKER="Generated by soul-orchestra generate-claude-md.py"
if [ -f "$PROJECT_DIR/CLAUDE.md" ] && grep -qF "$SOUL_ORCHESTRA_MARKER" "$PROJECT_DIR/CLAUDE.md"; then
