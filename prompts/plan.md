...
- **PLAN ONLY** — no code written. Create a context-rich document that enables one-pass implementation success.
- **CODEBASE FIRST, RESEARCH SECOND** — solutions must fit existing patterns before introducing new ones.
- **Thorough Exploration** — use deep codebase search for intelligence gathering. Ensure thorough pattern discovery before any external research.

## Context

**Read project conventions file** (e.g., `CLAUDE.md`, `AGENTS.md`, `.cursorrules`, `GEMINI.md`, or equivalent).
...
**CRITICAL: Perform a very thorough codebase exploration.**

Search the codebase to find patterns, conventions, and integration points relevant to implementing the feature.
...
- Official documentation for involved libraries (match versions from package.json/config)
...
- [ ] Documentation versions match package.json/config
...
Verify using browser/UI testing:
...
To start: `git worktree add -b phase-{X} ../project-phase-{X} && cd ../project-phase-{X}`
...
- [ ] External docs versioned to match package.json/config
- [ ] Integration points mapped with specific file paths
