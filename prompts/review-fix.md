...
PR number and optional flags: `{ARGS}`
...
|------------------|------|-------------|
| No `--severity`, no `--single-pass` | **LOOP (default)** | Fix all severities, then auto-invoke `{TOOL}:review`, loop until 0 issues or MAX_ROUNDS. Matches `prp-run-all` Step 6 semantics. |
...
If the input is a fix-summary artifact: STOP — "Cannot fix a fix-summary. Pass a review artifact instead (e.g., `pr-{N}-review-{TOOL}.md` or `pr-{N}-agents-review.md`)."

**Extract PR number from filename:**

```bash
# Artifact name format: pr-{NUMBER}-*.md (e.g. pr-123-review-{TOOL}.md)
...
```
Using review artifact: pr-123-review-{TOOL}.md
```
...
Run `{TOOL}:review {NUMBER}` first to generate the review.
```
...
- If SINGLE_PASS: `"Single-pass mode: will exit after Phase 6. Re-verify manually with {TOOL}:review {NUMBER} or enable loop by omitting --severity."`
...
**Skip this phase** if invoked as part of `{TOOL}:run-all` (detected by: `.prp-output/state/run-all.state.md` exists, or `[WORKSPACE CONTEXT]` present). The orchestrator produces its own summary in Step 7.
...
{If SINGLE_PASS and all issues matching the active severity set are fixed:} "Ready for re-review. Run `{TOOL}:review {NUMBER}` to verify."
...
|-------------|-------------------|----------------|
| All issues actually fixed | `false` | `{TOOL}:review {NUMBER} --since-last-review` (incremental, cheap) |
| Some issues skipped | `true`, `SKIPPED_COUNT = N` | `{TOOL}:review {NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `{TOOL}:review {NUMBER}` (FULL review — skipped items need to re-surface, incremental would miss them) |
| All issues skipped (no code change) | `true`, `ALL_SKIPPED = true` | `{TOOL}:review {NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `{TOOL}:review {NUMBER}` (FULL review), then evaluate Escalation Guard below |
...
Stop and tell user to run `{TOOL}:review {NUMBER}` first.
...
```
{TOOL}:review-fix {NUMBER} --severity critical,high,medium
```
...
```
{TOOL}:review-fix 163                                    # LOOP mode — fix all + re-review until 0 issues
{TOOL}:review-fix 163 --severity critical,high           # SINGLE-PASS — only critical and high
{TOOL}:review-fix 163 --single-pass                      # SINGLE-PASS explicit — fix all once, exit
{TOOL}:review-fix 163 --max-rounds 3                     # LOOP mode with custom cap (default 5)
{TOOL}:review-fix                                        # Current branch's PR, LOOP mode
{TOOL}:review-fix .prp-output/reviews/pr-42-review-{TOOL}.md  # By artifact path, LOOP mode
```
