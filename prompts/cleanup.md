...
```
DRY_RUN = true if "--dry-run" found in {ARGS}
ALL_MODE = true if "--all" found in {ARGS}
...
```
{TOOL}:cleanup                           # Current branch
{TOOL}:cleanup feat/auth                 # Specific branch
{TOOL}:cleanup --all                     # All merged branches
{TOOL}:cleanup --all --dry-run           # Preview batch cleanup
{TOOL}:cleanup feat/login --dry-run      # Preview single branch
```
