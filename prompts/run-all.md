...
if [ -f "$LOCK_FILE" ]; then
  ...
  # between -f check and stat) does not feed empty string into the
  # arithmetic and silently yield LOCK_AGE=0 (fresh lock).
  LOCK_MTIME=$(stat -c "%Y" "$LOCK_FILE" 2>/dev/null || stat -f "%m" "$LOCK_FILE" 2>/dev/null)
  if ! [[ "$LOCK_MTIME" =~ ^[0-9]+$ ]]; then
    # Lock file vanished or is unreadable — treat as stale.
    echo "STALE_LOCK"
  else
...
**STATE FILE I/O RULE**: Use **Bash with heredoc** (`cat > file << 'EOF'`) only for initial state/lock file creation or emergency recovery. For normal state mutations after creation, use the helper commands below so frontmatter and body updates remain validated and atomic.
...
Use `{TOOL}:implement` with PLAN_PATH.
...
Use `{TOOL}:ralph` with `{PLAN_PATH} --max-iterations {RALPH_MAX_ITER}`.
...
**CHECKPOINT**: Did you invoke `{TOOL}:implement` or `{TOOL}:ralph`? If not → STOP → invoke it.
...
**CHECKPOINT**: Did you invoke `{TOOL}:commit`? If not → STOP → invoke it.
...
**CHECKPOINT**: Did you invoke `{TOOL}:pr`? If not → STOP → invoke it.
...
Pass PR_NUMBER to the chosen command. If `.prp-output/reviews/pr-context-{BRANCH}.md` exists, pass `--context ".prp-output/reviews/pr-context-${BRANCH}.md"` for token optimization.
...
Use `{TOOL}:review-fix` with `{REVIEW_ARTIFACT} --severity {FIX_SEVERITY}`.
...
| `true` (some/all skipped) | `{TOOL}:review {PR_NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `{TOOL}:review {PR_NUMBER}` (FULL review) | Skipped items did NOT change code but STILL need to re-surface — incremental delta would miss them and produce false "0 issues" |

If `--since-last-review` not supported or fails, fall back to a full review with `--context ".prp-output/reviews/pr-context-${BRANCH}.md"` when that context file exists, or omit `--context` to let review gather fresh context. Display: `"WARN: --since-last-review not supported — falling back to full review (higher token cost)."`
...
- Create escalation GH issue with the remaining items. Label strategy: the `[escalation]` title prefix carries the signal — add repo-appropriate labels only if they exist in the target repo (graceful fallback so different repos with different label schemes do not hard-fail the workflow):
   ...
   if ! gh issue create \
     ...
     if mkdir -p .prp-output/reviews && printf '%s\n' "$ESCALATION_BODY" > "$ESCALATION_ARTIFACT"; then
       ...
       REVIEW_VERDICT="needs_manual_fix"
       exit 1
     fi
   fi
   ```
...
- Review: `.prp-output/reviews/pr-{PR_NUMBER}-review-{TOOL}.md`
...
{If NEEDS MANUAL FIXES: list remaining issues matching `FIX_SEVERITY`, including skipped issues}
...
4. **Pass context forward** — information flows from earlier to later steps. Pass `--context <path>` to review when a context file exists.
...
8. **Re-verify after fix** — always re-run `{TOOL}:review` after fix. Use `--since-last-review` only when review-fix skipped nothing; if any issue was skipped, run a full review with `--context <path>` when available.
...
{TOOL}:run-all Add JWT auth --ralph                     # Ralph autonomous loop
{TOOL}:run-all Add JWT auth --ralph --ralph-max-iter 5  # Ralph with 5 iterations
{TOOL}:run-all Add JWT auth --skip-review               # Skip review step
{TOOL}:run-all --prp-path plans/jwt.plan.md --no-pr     # Implement + commit only
{TOOL}:run-all Add JWT auth --no-interact               # Fully autonomous
...
| Some review-fix issues skipped | Re-verify with FULL review (`--context <path>` when a context file exists, not `--since-last-review`) so skipped items re-surface in next 6.2 evaluation. See Step 6.4 table. |
...
- CONTEXT_PASSED: Review context passed to review via `--context <path>` when available
...
- NO_SILENT_MERGE: `--merge` only executes when `REVIEW_VERDICT = "0_issues"`. `needs_manual_fix` (MAX_CYCLES hit OR 2 rounds all-skipped) blocks merge; escalation GH issue or local escalation artifact is created instead.
