...
Target URL and acceptance criteria: `{ARGS}`
...
**If `--delegate=<agent>` is present in `{ARGS}`**: extract the agent name and enter delegation mode.
...
Extract the target URL from `{ARGS}`. If no URL provided:
...
If no URL found: STOP — "No target URL provided and no local dev server detected. Provide a URL: `{TOOL}:qa <url> --criteria '...'`"
...
```
{TOOL}:qa http://localhost:3000/workflows --criteria "Filters work, mobile layout has no horizontal scroll, status badges show correct colors"
{TOOL}:qa http://localhost:3000/tasks --issue 422
{TOOL}:qa https://dashboard.goko.digital --plan .prp-output/plans/dashboard-plan.md
{TOOL}:qa http://localhost:8080/api/tasks --api-only --criteria "Returns JSON array, status 200, items have id+title+status fields"
{TOOL}:qa http://localhost:3000 --viewports desktop,mobile,tablet --criteria "Responsive layout, no overflow on mobile"
```
