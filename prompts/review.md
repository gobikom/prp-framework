...
REVIEW_FILE=$(ls -t .prp-output/reviews/pr-${NUMBER}-review-{TOOL}.md 2>/dev/null | head -1)
```
...
Save aggregated review to `.prp-output/reviews/pr-{NUMBER}-review-{TOOL}.md` before posting to GitHub.

> **Note**: Uses `-{TOOL}` suffix to identify which tool produced the review and prevent overwriting reviews from other tools (each tool uses its own suffix for parallel review capability).

### Post to GitHub

| Condition | Action |
|-----------|--------|
| READY TO MERGE | `gh pr review {NUMBER} --approve --body-file .prp-output/reviews/pr-{NUMBER}-review-{TOOL}.md` |
| NEEDS FIXES or CRITICAL ISSUES | `gh pr review {NUMBER} --request-changes --body-file .prp-output/reviews/pr-{NUMBER}-review-{TOOL}.md` |
| Draft PR | `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-review-{TOOL}.md` (comment only — no formal review on drafts) |
...
3. Fall back to: `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-review-{TOOL}.md`
...
**PR**: #{NUMBER}
**Verdict**: {READY TO MERGE / NEEDS FIXES / CRITICAL ISSUES}
**Review File**: `.prp-output/reviews/pr-{NUMBER}-review-{TOOL}.md`
...
```
{TOOL}:review 163                        # Full review
{TOOL}:review 163 tests errors           # Specific aspects
{TOOL}:review 163 security deps          # Security + dependency only
{TOOL}:review                            # Current branch's PR
{TOOL}:review 42 code docs              # Only code and docs
{TOOL}:review 42 simplify               # Just simplify
{TOOL}:review 42 perf a11y              # Performance + accessibility focus
{TOOL}:review 163 --since-last-review   # Incremental re-review
{TOOL}:review --metrics                  # View review metrics
```
