...
PR number and optional criteria: `{ARGS}`
...
Extract PR number from `{ARGS}`. If not provided:
...
```
{TOOL}:verify 42
{TOOL}:verify 42 --issue 166
{TOOL}:verify 42 --plan .prp-output/plans/feature.plan.md
{TOOL}:verify 42 --criteria "Gate 2 blocks agent PRs, bypass requires --reason"
```
