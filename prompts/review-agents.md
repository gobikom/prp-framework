...
| Draft PR or permission fallback | `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-agents-review.md` |
...
| Agent times out or fails | Note failure, proceed with available results, suggest single-session fallback for that aspect |
