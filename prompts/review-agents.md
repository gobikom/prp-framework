...
- **Skip Validation Phase** if context file already contains validation results
  (from `{TOOL}:implement` or `{TOOL}:ralph`).
...
**This command differs from `{TOOL}:review`** (single-session sequential passes) by spawning actual Agent subprocesses via the Agent tool. Each agent gets:
...
Before gathering PR information, check if context was pre-generated by `{TOOL}:implement` or `{TOOL}:ralph`:
...
First, resolve the PR number if not already known (from `{ARGS}` or `gh pr view --json number -q '.number'`).
...
Fall back to `{TOOL}:review` which performs all 11 review passes sequentially in a single session. Display:
...
> **Note**: Uses `-agents-review` suffix (distinct from `-review-claude-code.md` used by single-session `{TOOL}:review`) to prevent overwriting.
...
```
{TOOL}:review-agents 163                        # Full multi-agent review
{TOOL}:review-agents 163 security errors        # Only security + error handling agents
{TOOL}:review-agents                            # Current branch's PR
{TOOL}:review-agents 42 code docs               # Only code and docs agents
...
- FALLBACK_AVAILABLE: Graceful fallback to {TOOL}:review when Agent tool unavailable
