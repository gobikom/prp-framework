...
1. Check `--context <path>` (passed by run-all workflow when a context file exists)
...
When run via `run-all`, review receives a pre-generated context file via `--context <path>` when that file exists. This skips redundant file gathering and saves ~60K tokens. If the context file is not available, review proceeds normally without `--context`.
