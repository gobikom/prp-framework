...
run-all สร้าง state file `.claude/prp-run-all.state.md` เพื่อ track progress ระหว่าง run — ถ้า session crash สามารถใช้ `--resume` เพื่อ resume จาก step ที่ fail ได้ State file จะถูกลบอัตโนมัติเมื่อ workflow สำเร็จ

**Review-Fix Loop**:

หลัง review จะเข้า fix loop สูงสุด 2 รอบ — fix ทุก issues ที่ตรงกับ `FIX_SEVERITY` (default: critical, high, medium, suggestion ทุก level) ถ้ายังมี issues ที่แก้ไม่ได้หลังจาก 2 รอบจะหยุดและ report ว่าต้อง manual fix

**Token Optimization** (v2.1.0):

- Re-verify (Step 6.4) ใช้ `--since-last-review` สำหรับ incremental review — ประหยัด tokens โดย review เฉพาะ changes หลัง fix
...
6. **Remove orphaned state files** (`.claude/prp-run-all.state.md` if references cleaned branch)
...
> **หมายเหตุ**: Config นี้ scope `rm` ให้ลบได้เฉพาะ PRP state files (`.claude/prp-*`) และ artifacts (`.prp-output/*`) และ scope `sed` ให้แก้ไขได้เฉพาะ artifacts เท่านั้น
