...
**กระบวนการ**:
1. Load review artifact จาก `.prp-output/reviews/pr-{NUMBER}-review*.md`
2. ถ้ามีหลาย artifact (หลาย tool review) — แสดง list ให้เลือก
...
หลัง review จะเข้า fix loop สูงสุด 2 รอบ — ถ้ายังมี critical/high issues หลังจาก 2 รอบจะหยุดและ report ว่าต้อง manual fix
...
เพิ่ม config นี้ใน **project** `.claude/settings.json` (commit เข้า git ได้):
...
> **หมายเหตุ**: Config นี้ครอบคลุม commands ที่ PRP workflows ใช้ทั้งหมด — git operations, GitHub CLI, file management, build tools ลบ tools ที่โปรเจกต์ไม่ใช้ออกได้ (เช่น ถ้าไม่ใช้ bun ก็เอาออก)
