...
3. **Installed adapter targets:**
   ...
   **Exception — soul-orchestra agent homes:** If `PROJECT_DIR/CLAUDE.md` exists and contains the header `<!-- Generated by soul-orchestra generate-claude-md.py -->`, the `AGENTS.md` install step is **skipped**. These agent homes (e.g. `~/repos/agents/agent-psak/`, `~/repos/agents/agent-dora/`) own their root `AGENTS.md` as codex-loadable agent identity, parallel to their auto-generated `CLAUDE.md`. Installing the generic PRP workflow help at root `AGENTS.md` would overwrite the identity file on every `prp-install-all` run. All other adapters (`.codex/skills/prp-*`, etc.) continue to install normally — the skip is scoped narrowly to the root `AGENTS.md` path only.
