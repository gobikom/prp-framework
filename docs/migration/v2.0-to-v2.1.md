...
> **Note (updated v2.3.0)**: Both `review-agents` and `feature-review-agents` are now standalone commands across all 5 adapters that spawn parallel Agent subprocesses via the Agent tool. They are no longer Claude Code-exclusive or aliases. `design` also parallelizes its exploration and research phases.
...
Additionally, `review-agents` and `feature-review-agents` are now standalone parallel-agent commands across all 5 adapters (updated in v2.3.0). `design` also parallelizes its exploration and research phases. Use `review` and `feature-review` for single-session sequential alternatives.
