...
> **Note (updated v2.3.0)**: `review-agents` is now a standalone command across all 5 adapters that spawns parallel Agent subprocesses via the Agent tool. It is no longer Claude Code-exclusive or an alias. `feature-review-agents` remains an alias for `feature-review`.
