...
7. **GENERATE**: Save PRD to `.prp-output/prds/drafts/{kebab-case-name}-prd-antigravity.md` (create directory: `mkdir -p .prp-output/prds/drafts`) with ALL sections:

   > **Note**: Uses `-antigravity` suffix to identify Antigravity PRD drafts. Multiple tools can create draft PRDs in `drafts/` subdirectory for comparison. User manually merges best sections to final version at `.prp-output/prds/{name}-prd.md` (no suffix, root level) which Plan command will reference.
