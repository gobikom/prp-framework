...
|----------------|-----------------|---------------|------------|--------|
| "Where's my order?" | order.status | order.status | 0.95 | PASS |
| "Track package" | order.status | shipping.info | 0.72 | FIX |
| "Whrre is my ordr" | order.status | order.status | 0.85 | PASS (typo handled) |
| "asdfghjkl" | fallback | fallback | N/A | PASS |
