...
REVIEW_FILE=$(ls -t .prp-output/reviews/pr-${NUMBER}-review-antigravity.md 2>/dev/null | head -1)
```
...
| Draft PR | `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-review-antigravity.md` (comment only — no formal review on drafts) |
...
3. Fall back to: `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-review-antigravity.md`
...
**PR**: #{NUMBER}
**Verdict**: {READY TO MERGE / NEEDS FIXES / CRITICAL ISSUES}
**Review File**: `.prp-output/reviews/pr-{NUMBER}-review-antigravity.md`
