...
If the input is a fix-summary artifact: STOP — "Cannot fix a fix-summary. Pass a review artifact instead (e.g., `pr-{N}-review-antigravity.md` or `pr-{N}-agents-review.md`)."

**Extract PR number from filename:**

```bash
# Artifact name format: pr-{NUMBER}-*.md (e.g. pr-123-review-antigravity.md)
...
```
.prp-output/reviews/pr-123-review-antigravity.md   <- this tool's review
...
```
Using review artifact: pr-123-review-antigravity.md
```
...
Multiple review artifacts found for PR #123:
  ...
  [1] pr-123-review-antigravity.md    (this tool)     modified: 2026-02-27 14:30
...
| Some issues skipped | `true`, `SKIPPED_COUNT = N` | `/prp-review {NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `/prp-review {NUMBER}` (FULL review — skipped items need to re-surface, incremental would miss them) |
| All issues skipped (no code change) | `true`, `ALL_SKIPPED = true` | `/prp-review {NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `/prp-review {NUMBER}` (FULL review), then evaluate Escalation Guard below |
...
/prp-review-fix .prp-output/reviews/pr-42-review-antigravity.md  # By artifact path, LOOP mode
```
