...
| `/prp-review` | `.prp-output/reviews/pr-{PR_NUMBER}-review-antigravity.md` |
...
| `true` (some/all skipped) | `/prp-review {PR_NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `/prp-review {PR_NUMBER}` (FULL review) | Skipped items did NOT change code but STILL need to re-surface — incremental delta would miss them and produce false "0 issues" |
...
- Review: `.prp-output/reviews/pr-{PR_NUMBER}-review-antigravity.md`
...
8. **Re-verify after fix** — always re-run `/prp-review` after fix. Use `--since-last-review` only when review-fix skipped nothing; if any issue was skipped, run a full review with `--context <path>` when available.
