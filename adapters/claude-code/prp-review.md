...
## Phase 6.5: UPDATE ARTIFACTS - Document Feedback Loop

### 6.5.1 Update Implementation Report

**If implementation report was found in Phase 2.2:**

Append a "Review Outcome" section to the end of the implementation report file:
...
{If REQUEST CHANGES or BLOCK: brief summary of top blocking issues}
```

### 6.5.2 Update PRD (if applicable)

**If implementation report references a Source PRD:**

| Recommendation | PRD Update |
|----------------|------------|
| APPROVE | Change phase status from `complete` to `reviewed` |
| REQUEST CHANGES | Add note: "Review: {N} issues to address" to phase row |
| BLOCK | Add note: "Blocked: {brief reason}" to phase row |

**PHASE_6.5_CHECKPOINT:**
- [ ] Implementation report updated with review outcome (if report exists)
