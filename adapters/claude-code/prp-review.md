...
REVIEW_FILE=$(ls -t .prp-output/reviews/pr-${NUMBER}-review-claude-code.md 2>/dev/null | head -1)
```
...
# Structural diff (compact overview — use prp-diff if available)
...
Save aggregated review to `.prp-output/reviews/pr-{NUMBER}-review-claude-code.md` before posting to GitHub.

> **Note**: Uses `-claude-code` suffix to identify which tool produced the review and prevent overwriting reviews from other tools (each tool uses its own suffix for parallel review capability).

### Post to GitHub

| Condition | Action |
|-----------|--------|
| READY TO MERGE | `gh pr review {NUMBER} --approve --body-file .prp-output/reviews/pr-{NUMBER}-review-claude-code.md` |
| NEEDS FIXES or CRITICAL ISSUES | `gh pr review {NUMBER} --request-changes --body-file .prp-output/reviews/pr-{NUMBER}-review-claude-code.md` |
| Draft PR | `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-review-claude-code.md` (comment only — no formal review on drafts) |
...
3. Fall back to: `gh pr comment {NUMBER} --body-file .prp-output/reviews/pr-{NUMBER}-review-claude-code.md`
...
**PR**: #{NUMBER}
**Verdict**: {READY TO MERGE / NEEDS FIXES / CRITICAL ISSUES}
**Review File**: `.prp-output/reviews/pr-{NUMBER}-review-claude-code.md`
...
**Before creating PR**:
1. Run `/prp-core:prp-review` on current branch
2. Fix critical and important issues
3. Re-run to verify
4. Create PR

**During PR review**:
1. Run `/prp-core:prp-review <pr-number>`
2. Review posts summary to GitHub with formal approve/request-changes
3. Address feedback
4. Re-run targeted aspects

**After making changes**:
1. Run specific aspects: `/prp-core:prp-review <pr-number> tests code`
...
```
/prp-core:prp-review 163                        # Full review
/prp-core:prp-review 163 tests errors           # Specific aspects
/prp-core:prp-review 163 security deps          # Security + dependency only
/prp-core:prp-review                            # Current branch's PR
/prp-core:prp-review 42 code docs              # Only code and docs
/prp-core:prp-review 42 simplify               # Just simplify
/prp-core:prp-review 42 perf a11y              # Performance + accessibility focus
/prp-core:prp-review 163 --since-last-review   # Incremental re-review
/prp-core:prp-review --metrics                  # View review metrics
```
