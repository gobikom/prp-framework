...
**This command differs from `/prp-core:prp-feature-review`** (single-session sequential analysis) by spawning actual Agent subprocesses via the Agent tool. Each agent gets:
...
Fall back to `/prp-core:prp-feature-review` which performs all analysis sequentially in a single session. Display:
...
Running: /prp-core:prp-feature-review {PACKAGE_PATH}
```
...
| Agent tool not available | Fall back to `/prp-core:prp-feature-review` |
...
- FALLBACK_AVAILABLE: Graceful fallback to /prp-core:prp-feature-review
