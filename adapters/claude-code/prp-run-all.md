...
| `--verify` | Run /prp-core:prp-verify before merge to check requirements traceability (Step 7.5). Set VERIFY = true. |
| `--qa-delegate=<agent>` | Delegate QA to agent after merge via /prp-core:prp-qa --delegate (Step 8.5). Set QA_DELEGATE = agent. |
| `--done` | Run /prp-core:prp-done after QA delegation to close issue when all gates pass (Step 9). Set DONE = true. Requires --issue. |
...
Steps that would run:
  ...
  Step 7.5: Verify             -> {skipped (no --verify) | /prp-core:prp-verify --pr {PR_NUMBER} --issue {ISSUE_NUMBER}}
  Step 8: Merge & Cleanup      -> {skipped (no --merge) | squash merge + cleanup}
  Step 8.5: QA Delegate        -> {skipped (no --qa-delegate) | /prp-core:prp-qa --delegate={QA_DELEGATE} --issue {ISSUE_NUMBER}}
  Step 9: Done                 -> {skipped (no --done or no --issue) | /prp-core:prp-done {ISSUE_NUMBER}}
...
| Verify | /prp-core:prp-verify | {PASS/FAIL/PARTIAL or "skipped (no --verify)"} |
| Merge | gh pr merge --squash | {pending (Step 8) or "skipped (no --merge)"} |
| Cleanup | /prp-core:prp-cleanup | {pending (Step 8) or "skipped"} |
| QA Delegate | /prp-core:prp-qa --delegate | {dispatched to {QA_DELEGATE} or "skipped (no --qa-delegate)"} |
| Done | /prp-core:prp-done | {closed #{ISSUE_NUMBER} or "skipped" or "pending QA"} |
...
Use `/prp-core:prp-verify` with `--pr {PR_NUMBER}` (append `--issue {ISSUE_NUMBER}` if set, else `--plan {PLAN_PATH}`).
...
**CHECKPOINT**: Did you invoke `/prp-core:prp-verify`? If not → STOP → invoke it.
...
Use `/prp-core:prp-qa --delegate={QA_DELEGATE}` with `--issue {ISSUE_NUMBER}` (required — QA needs criteria source).
...
**CHECKPOINT**: Did you invoke `/prp-core:prp-qa --delegate={QA_DELEGATE}`? If not → STOP → invoke it.
...
**Note**: If Step 8.5 delegated QA, Step 9 will likely block on Gate 4 (QA PENDING). That is expected — run `/prp-core:prp-done {ISSUE_NUMBER}` again after Vera QA posts results.

Use `/prp-core:prp-done {ISSUE_NUMBER}`.
...
| Gate 4 pending (QA delegated but not done) | WARN: "QA not yet complete — issue NOT closed. Re-run `/prp-core:prp-done {ISSUE_NUMBER}` after Vera posts QA results." |
| Any other gate fails | STOP — report failed gates. Issue NOT closed. |

**DO NOT**: Close the issue directly with gh, skip the skill.
**CHECKPOINT**: Did you invoke `/prp-core:prp-done`? If not → STOP → invoke it.
