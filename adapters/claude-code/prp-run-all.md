...
| `true` (some/all skipped) | `/prp-core:prp-review {PR_NUMBER} --context ".prp-output/reviews/pr-context-${BRANCH}.md"` when the context file exists, otherwise `/prp-core:prp-review {PR_NUMBER}` (FULL review) | Skipped items did NOT change code but STILL need to re-surface — incremental delta would miss them and produce false "0 issues" |
