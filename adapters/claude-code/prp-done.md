---
description: "Issue closure gate - checks all completion gates before closing (PR merged, review, verify, Vera QA)"
argument-hint: "<issue-number> [--repo <owner/repo>] [--skip-vera --reason=\"...\"]"
---
<process>
...
Extract issue number from `$ARGUMENTS`. If not provided: STOP — "Issue number required. Usage: `/prp-core:prp-done <issue-number>`"
...
|--------|--------|
| File found | ✅ PASS |
| No file | ❌ FAIL — "No review artifact for PR #${PR_NUMBER}. Run: /prp-core:prp-review-agents ${PR_NUMBER}" |
...
|--------|--------|
| File found | ✅ PASS |
| No file | ❌ FAIL — "No verify artifact for PR #${PR_NUMBER}. Run: /prp-core:prp-verify ${PR_NUMBER} --issue ${ISSUE_NUMBER}" |
...
| "QA FAILED" found (last match) | ❌ FAIL — "Vera QA failed. Fix and re-delegate: /prp-core:prp-qa --delegate=vera --issue #{N}" |
| No QA comment | ⏳ PENDING — "Vera QA not completed. Delegate: /prp-core:prp-qa --delegate=vera --issue #{N}" |
...
To fix:
  /prp-core:prp-verify M --issue N
  /prp-core:prp-qa --delegate=vera --issue N
```
...
```
/prp-core:prp-done 166
/prp-core:prp-done 166 --repo gobikom/agent-devops
/prp-core:prp-done 166 --skip-vera --reason="no UI changes — API-only fix"
```
