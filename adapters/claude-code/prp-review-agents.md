...
- **Skip Validation Phase** if context file already contains validation results
  (from `/prp-core:prp-implement` or `/prp-core:prp-ralph`).
...
**This command differs from `/prp-core:prp-review`** (single-session sequential passes) by spawning actual Agent subprocesses via the Agent tool. Each agent gets:
...
Before gathering PR information, check if context was pre-generated by `/prp-core:prp-implement` or `/prp-core:prp-ralph`:
...
- Per-file checklist: Correctness, Type Safety, Pattern Compliance, Completeness, Maintainability, Scope (unexplained hunks = High)
...
Fall back to `/prp-core:prp-review` which performs all 11 review passes sequentially in a single session. Display:
...
Running: /prp-core:prp-review {NUMBER}
```
...
Consider running /prp-core:prp-review {NUMBER} {aspect} for a single-session {aspect} review.
```
...
> **Note**: Uses `-agents-review` suffix (distinct from `-review-claude-code.md` used by single-session `/prp-core:prp-review`) to prevent overwriting.
...
```
/prp-core:prp-review-agents 163                        # Full multi-agent review
/prp-core:prp-review-agents 163 security errors        # Only security + error handling agents
/prp-core:prp-review-agents                            # Current branch's PR
/prp-core:prp-review-agents 42 code docs               # Only code and docs agents
...
- FALLBACK_AVAILABLE: Graceful fallback to /prp-core:prp-review when Agent tool unavailable
