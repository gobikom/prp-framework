---
name: prp-qa
description: "Acceptance criteria verification — Playwright browser testing, API contract testing, E2E flows, responsive viewports, and accessibility checks. Reports pass/fail per criterion with screenshot evidence."
metadata:
  short-description: QA acceptance testing
---
...
If no URL found: STOP — "No target URL provided and no local dev server detected. Provide a URL: `$prp-qa <url> --criteria '...'`"
...
```
$prp-qa http://localhost:3000/workflows --criteria "Filters work, mobile layout has no horizontal scroll, status badges show correct colors"
$prp-qa http://localhost:3000/tasks --issue 422
$prp-qa https://dashboard.goko.digital --plan .prp-output/plans/dashboard-plan.md
$prp-qa http://localhost:8080/api/tasks --api-only --criteria "Returns JSON array, status 200, items have id+title+status fields"
$prp-qa http://localhost:3000 --viewports desktop,mobile,tablet --criteria "Responsive layout, no overflow on mobile"
```
